...
public class BootstrapTableTest {
    ...
    public void editRow() throws InterruptedException {
        ...
        driver.findElement(By.xpath("//input[contains(@placeholder, 'First')]")).sendKeys("Dimitrije");
        driver.findElement(By.xpath("//input[contains(@placeholder, 'Last')]")).clear();
        driver.findElement(By.xpath("//input[contains(@placeholder, 'Last')]")).sendKeys("Petronijevic");
        driver.findElement(By.xpath("//input[contains(@placeholder, 'Middle')]")).clear();
        driver.findElement(By.xpath("//input[contains(@placeholder, 'Middle')]")).sendKeys("Dejan");

        Thread.sleep(1000);
        driver.findElement(By.id("up")).click();
        Assert.assertEquals("Dimitrije",
                "Dimitrije",
                "[ERROR] This is not the right input");
        Assert.assertEquals("Petronijevic",
                "Petronijevic",
                "[ERROR] This is not the right input");
        Assert.assertEquals("Dejan",
                "Dejan",
                "[ERROR] This is not the right input");
    }
